...
interface UpdateSubjectsModalProps {
  ...
  onSubmit: (subjectId: string, subject: Partial<Subject>) => void;
  isLoading?: boolean;
  courses: { courseId: string; title: string }[];
  semesters: { semesterId: string; name: string }[];
}

export default function UpdateSubjectsModal({ 
  isOpen, 
  onClose, 
  subject,
  onSubmit,
//   isLoading = false,
  ...
  // Load subject data when modal opens
  useEffect(() => {
    if (subject && isOpen) {
      setFormData({
        code: subject.code || "",
        ...
        creditUnits: subject.creditUnits || 0,
        semesterId: subject.semesterId || "",
        yearLevel: subject.yearLevel || 1,
        type: subject.type || "MAJOR",
        isShared: subject.isShared || false,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await onSubmit(subject.subjectId, formData);
      onClose();
    } catch (error) {
  ...
  return (
    <Modal
      ...
      aria-labelledby="update-subject-modal-title"
    >
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <select
                        ...
                        value={formData.type}
                        onChange={handleChange}
                        className="w-full px-3 py-2.5 border rounded-lg text-sm appearance-none
                        dark:text-gray-200 
                          bg-white dark:bg-white/5 transition-colors duration-150
                          focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2
                          border-gray-200 hover:border-gray-300"
          ...
          <div className="flex flex-col-reverse sm:flex-row items-center justify-between gap-3 sm:gap-0">
            ...
            <div className="flex items-center gap-2 sm:gap-3 w-full sm:w-auto justify-center sm:justify-end">
              ...
              <button 
                ...
                disabled={isSubmitting || !formData.code || !formData.name || !formData.courseId || !formData.semesterId}
                className={`
                  ...
                  ${isSubmitting || !formData.code || !formData.name || !formData.courseId || !formData.semesterId
